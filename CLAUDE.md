...
- **Routes** — `/` is the Workspace; `/settings/skills` and `/settings/connectors` are the full-width **Extensions Center** (`features/extensions/`), which keeps `SessionSidebar` and replaces everything right of it. Browse state lives in the URL (`?tab=` / `?q=` / `?import=1` / `?new=1`), so refresh and back/forward keep their place; clicking any session navigates back to `/`. Skill and MCP administration lives *only* here — there is no manager modal and no "系统集成" settings section any more. The 推荐 / SkillHub / 可用连接器 browse views render `extensions/catalog/staticCatalog.ts`, a **local static list** behind `VITE_EXTENSION_CATALOG` (default `off`) — there is no `/skillhub` or `/mcp/catalog` endpoint. Session list + CRUD is shared by both routes through `hooks/useWorkspaceSessions.ts`; the sidebar is still rendered once per route (`TODO(shell)`).
