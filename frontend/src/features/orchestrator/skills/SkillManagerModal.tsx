...
export function SkillManagerModal({
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="技能 · 连接器管理"
      subtitle="所有数据来自当前 Runtime /skills 接口"
    ...
    >
      <label className="mb-[16px] flex items-center gap-[8px] rounded-md border border-border-subtle bg-surface-2 px-[12px] py-[7px] text-text-tertiary transition-colors focus-within:border-accent">
        ...
        <input
          ...
          placeholder="搜索技能…"
          className="min-w-0 flex-1 bg-transparent text-body-sm text-text-primary outline-none placeholder:text-text-tertiary"
        />
      </label>
      ...
      {error && (
        <p
          role="alert"
          className="mb-[12px] rounded-md bg-status-error-bg p-[12px] text-caption text-status-error"
        >
          {error}
        </p>
      )}
      ...
      <div className="grid grid-cols-2 gap-[12px] max-sm:grid-cols-1">
        {filtered.map((skill) => {
          // `valid` is true for a package that was simply never trusted; only
          // `level` says the user actually granted it.
          const trusted = trust[skill.name] ?? skill.trust?.level === 'user_trusted';
          // A stale record means the package changed since it was trusted.
          const staleTrust = trust[skill.name] === undefined && skill.trust?.valid === false;
          ...
          return (
            ...
            >
              <h3 className="m-0 mb-[4px] truncate text-[13.5px] font-medium text-text-primary">
                {skill.display_name ?? skill.name}
              </h3>
              <p className="mb-[10px] line-clamp-3 min-h-[36px] text-[11.5px] leading-[1.55] text-text-secondary">
                {skill.description}
              </p>
              <div className="flex flex-wrap items-center gap-[8px]">
                {skill.scripts && skill.scripts.length > 0 && (
                  <Badge tone="planning">Scripts ×{skill.scripts.length}</Badge>
                )}
                <Badge tone={trusted ? 'success' : 'warning'}>
                  {trusted ? '已信任' : staleTrust ? '技能已变更，信任失效' : '未信任'}
                </Badge>
                {trusted ? (
                  <button
                    type="button"
                    ...
                    onClick={() =>
                      void mutate(
                        skill.name,
                        () => revokeSkillTrust(skill.name),
                        () => setTrust((current) => ({ ...current, [skill.name]: false })),
                      )
                    }
                  ...
                  <button
                    ...
                    onClick={() =>
                      void mutate(
                        skill.name,
                        () => trustSkill(skill.name, true),
                        () => setTrust((current) => ({ ...current, [skill.name]: true })),
                      )
                    }
                ...
                <button
                  ...
                  title="删除已导入技能"
                  onClick={() => {
                    if (window.confirm(`仅删除已导入技能“${skill.name}”？`))
                      void mutate(skill.name, () => deleteSkill(skill.name));
                  }}
      ...
      {!loading && filtered.length === 0 && (
        <p className="py-[40px] text-center text-caption text-text-tertiary">没有匹配的技能</p>
      )}
      {loading && (
        <p role="status" className="py-[40px] text-center text-caption text-text-tertiary">
          正在加载技能…
        </p>
      )}
      <p className="mt-[16px] text-caption text-text-tertiary">
        注：当前 GET /skills
        未返回进程信任状态；本面板只展示本次操作得到的真实响应，不伪造重启后的状态。
      </p>
    </Modal>
  );
}
