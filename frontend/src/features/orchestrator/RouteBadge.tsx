  ...
  return (
    <span className="inline-flex items-center gap-2">
      <span className="inline-flex flex-col items-end gap-[3px]">
        <span className="text-[10px] font-semibold uppercase tracking-eyebrow text-text-tertiary">
          置信度
        </span>
        <ProgressBar percent={value * 100} fillClassName={meta.dot} className="w-[52px]" />
      </span>
      <span className={`min-w-[34px] text-right font-mono text-mono font-semibold ${meta.fg}`}>
        {value.toFixed(2)}
      </span>
    </span>
  );
...
export function RouteBadge({
  ...
  return (
    <div
      className={`flex flex-col gap-2 rounded-md border border-l-2 px-[13px] py-[10px] ${meta.tintBg} ${meta.border} ${meta.leftBorder}`}
    >
      ...
      {reason && (
        <div className="flex gap-[7px] pl-[18px]">
          <span className="flex-none pt-[1px] text-[10px] font-semibold uppercase tracking-[0.08em] text-text-tertiary">
            理由
          </span>
