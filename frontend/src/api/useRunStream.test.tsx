...
const mocks = vi.hoisted(() => ({ streamRun: vi.fn(), createRun: vi.fn(), getRun: vi.fn(), cancelRun: vi.fn(), resolveApproval: vi.fn(), uploadArtifact: vi.fn() }));
vi.mock('./runs', () => ({
...
beforeEach(() => {
  ...
  mocks.createRun.mockReset().mockResolvedValue({ run_id: 'r1', session_id: 's1', objective: 'x', path: 'fast', status: 'running_fast', steps: {}, pending_approvals: [], revision: 0 });
  mocks.getRun.mockReset().mockImplementation(async (id: string) => ({ run_id: id, session_id: 's1', objective: 'restored', path: 'structured', status: 'waiting_approval', steps: {}, pending_approvals: [{ approval_id: 'a1' }], revision: 2 }));
...
describe('useRunStream', () => {
  ...
  it('records an unknown parsed event as a diagnostic', async () => {
    mocks.streamRun.mockImplementationOnce(async function* () { yield { event_id: 'x', type: 'future.event', data: {}, unknown: true }; yield { event_id: 'y', type: 'run.completed', data: {} }; });
    const { result } = renderHook(() => useRunStream('s1'));
    await act(async () => { await result.current.start('x', [], []); });
  ...
  it('clears the prior run when the active session changes', async () => {
    mocks.streamRun.mockImplementationOnce(async function* () { yield { event_id: '1', type: 'run.completed', data: { final_text: 'done' } }; });
  ...
  it('does not let a late restore overwrite the newly selected session', async () => {
    ...
    mocks.getRun.mockImplementationOnce(() => new Promise((resolve) => { release = resolve; }));
    const { result, rerender } = renderHook(({ sessionId }) => useRunStream(sessionId), { initialProps: { sessionId: 's1' } });
    let pending!: Promise<void>;
    act(() => { pending = result.current.restore('old-run'); });
    rerender({ sessionId: 's2' });
    release({ run_id: 'old-run', session_id: 's1', objective: 'old', path: 'fast', status: 'completed', steps: {}, pending_approvals: [], revision: 1 });
  ...
  it('uploads attachments and submits their real artifact ids with selected skills', async () => {
    mocks.uploadArtifact.mockResolvedValue({ artifact_id: 'artifact-1', sha256: 'sha', media_type: 'text/plain', bytes: 1 });
    mocks.streamRun.mockImplementationOnce(async function* () { yield { event_id: '1', type: 'run.completed', data: {} }; });
    const { result } = renderHook(() => useRunStream('s1'));
    await act(async () => { await result.current.start('inspect', [new File(['x'], 'input.txt')], ['quality'], true); });
    expect(mocks.createRun).toHaveBeenCalledWith(expect.objectContaining({ source: 'expert', skill_names: ['quality'], artifact_ids: ['artifact-1'] }));
  });

  it('surfaces upload failures without creating a run', async () => {
    mocks.uploadArtifact.mockRejectedValue(new Error('附件上传失败'));
    const { result } = renderHook(() => useRunStream('s1'));
    await act(async () => { await expect(result.current.start('inspect', [new File(['x'], 'input.txt')], [])).rejects.toThrow('附件上传失败'); });
  ...
  it('cancels the active run and projects the cancelled snapshot', async () => {
    mocks.streamRun.mockImplementationOnce(async function* () { yield { event_id: 'streaming', type: 'token.delta', data: { delta: '' } }; await new Promise(() => undefined); });
    mocks.cancelRun.mockResolvedValue({ run_id: 'r1', session_id: 's1', objective: 'x', path: 'fast', status: 'cancelled', steps: {}, pending_approvals: [], revision: 2 });
    const { result } = renderHook(() => useRunStream('s1'));
    await act(async () => { await result.current.start('x', [], []); });
  ...
  it('submits approval with the snapshot revision and reconnects the stream', async () => {
    mocks.resolveApproval.mockResolvedValue({ run_id: 'pending-run', session_id: 's1', objective: 'restored', path: 'structured', status: 'running_structured', steps: {}, pending_approvals: [], revision: 3 });
    mocks.streamRun.mockImplementationOnce(async function* () { yield { event_id: 'done', type: 'run.completed', data: { final_text: 'done' } }; });
    const { result } = renderHook(() => useRunStream('s1'));
    await act(async () => { await result.current.restore('pending-run'); });
    await act(async () => { await result.current.approve('a1', false, 2); });
    expect(mocks.resolveApproval).toHaveBeenCalledWith('pending-run', 'a1', false, 2);
    await vi.waitFor(() => expect(mocks.streamRun).toHaveBeenCalledWith('pending-run', undefined, expect.any(AbortSignal)));
  });
  ...
  it('keeps a pending approval intact when the revision request fails', async () => {
    ...
    const { result } = renderHook(() => useRunStream('s1'));
    await act(async () => { await result.current.restore('pending-run'); });
    await act(async () => { await expect(result.current.approve('a1', true, 1)).rejects.toThrow('stale approval'); });
