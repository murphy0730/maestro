...
import { createSession, listSessions, useSkills } from '@/api';
import { useRunStream } from '@/api/useRunStream';
import { useRunStore } from '@/stores/runStore';
import type { SkillMeta } from '@/types';

export function Workspace() {
  const [clock, setClock] = useState('--:--:--'); const [sessionId, setSessionId] = useState('default'); const [expert, setExpert] = useState(false); const [skills, setSkills] = useState<SkillMeta[]>([]); const [approvingId, setApprovingId] = useState<string | null>(null); const [importOpen, setImportOpen] = useState(false);
  ...
  const { start, approve, cancel, transport } = useRunStream(sessionId);
