...
- 多 Skill、跨系统、长时间、后台执行或高风险任务进入受控执行模式。
- 快速路径在发现风险或复杂度升级信号后，只能单向升级到受控执行模式。
- 两种模式共享安全和运行底座，不能通过选择模式绕过权限、审批、预算或审计。
...
flowchart TD
    ...
    RC -->|复杂、长时、高风险| CE[Controlled Execution]
...
受控执行模式不生成预先构建的目标契约或 DAG。Coordinator 每轮只接受一个模型动作，并使用更严格的步数、时间、循环和上下文预算。每个动作在执行前仍由 Capability Runtime 与 Policy Gate 验证；高风险写入经审批，未知写入结果先对账，取消和恢复均以 Journal 事实为准。
...
快速路径不生成预先构建的计划，但必须具备 `RunIntent`、Journal、Policy Gate、预算、取消和恢复能力。
...
- fork/Child Run 协作；
- 显式高风险或影响范围较大的动作；
- 需要严格审批、对账、取消、恢复或补偿治理。

受控执行模式逐轮处理模型动作；它不创建 DAG、拓扑顺序或计划级依赖语义。
...
4. 保留已冻结工作集、当前上下文、审批和能力快照；
5. 以受控预算重新校验策略后继续。

受控执行模式不能降回快速路径。模型不能通过重新描述任务降低 Tool/MCP 的确定性风险等级。
...
Runtime Core 不在 Skill frontmatter 中新增 Maestro 专属的 `risk-level`、`result-schema`、`preconditions`、`idempotency` 或 `success-criteria` 字段。此类执行和安全语义属于 Tool/MCP、Policy、RunIntent 和当前受控执行状态。
...
4. 快速路径直接进入有界动态循环；受控执行模式以更严格预算逐轮处理动作。
...
`structured`、`structuring` 和 `running_structured` 是稳定的持久化/API 值，分别表示受控执行模式、进入该模式和在该模式中运行；它们不表示目标规范、计划图或拓扑执行。
...
所有状态迁移必须由 Coordinator 执行并写入 Journal。状态机应拒绝非法回退，例如从受控执行模式回到快速路径，或从已完成步骤重新进入运行状态。
...
Run 启动后固定所用 Skill、Tool、MCP Provider 的版本或内容摘要。运行中发现版本变化时，不静默切换；应暂停并重新评估当前受控执行状态或要求确认，防止语义漂移。
...
- RunIntent 能确定性选择快速或受控执行初始模式；
- 快速路径可安全单向升级，且上下文和运行身份连续；
- 两种模式共用 Policy、Journal、预算、取消和事件协议；
- Runtime 不生成或重放预先构建的目标规范、计划图或拓扑依赖；
