...
- 日期：2026-07-07
- 状态：设计已确认，待实现计划
- 适用范围：把 `scheduling_platform`（Python FastAPI 后端）+ `frontend`（React/Electron 壳）打包为 Windows 与 macOS 可运行的桌面应用
...
- `scheduling_platform/.../config.py`：`port` 读 `MAESTRO_BACKEND_PORT`，绑定改 `127.0.0.1`（~+5 行）。
...
### 5.1 新增后端入口（冻结用）
`scheduling_platform/src/scheduling_platform/sidecar_entry.py`（~30 行）：从 env 读 `MAESTRO_BACKEND_PORT`，`from scheduling_platform.main import app` 后 `uvicorn.run(app, host="127.0.0.1", port=port, workers=1, reload=False)`。stdout/stderr 由 Electron 重定向到 `userData/backend.log` 供排障。这是 PyInstaller 的 entrypoint（不用 `cli`，那是交互式）。

### 5.2 PyInstaller（onefolder）
spec `scheduling_platform/maestro_backend.spec`，关键项：
...
- 产物名 `MaestroBackend`（mac）/ `MaestroBackend.exe`（win），输出到 `scheduling_platform/dist/backend/`
...
```jsonc
"extraResources": [
  { "from": "../scheduling_platform/dist/backend", "to": "backend", "filter": ["**/*"] }
]
```
...
codesign --deep --force --options runtime --sign "Developer ID Application: <name>" \
  scheduling_platform/dist/backend
```
