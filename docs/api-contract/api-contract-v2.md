...
| `GET /extension-catalog/sources|status|skills|connectors` | ✅ | 扩展目录查询与同步状态（见 §7.1） |
| `POST /extension-catalog/*` | ✅ | 手动同步、Skill 安装更新、MCP 添加更新；需要 Bearer 管理凭证 |
...
`guarded_host`。当前信任主体来自 privileged API 会话，Electron 默认使用本机管理员主体。
...
POST /extension-catalog/skills/{catalog_id}/install|update
POST /extension-catalog/connectors/{catalog_id}/add|update
```

管理端点以及既有 Skill/MCP mutation 使用 `Authorization: Bearer <token>`。Electron 每次启动生成随机 Token；本地 `restart.sh` 注入开发 Token；Web 部署必须配置强随机 `PRIVILEGED_API_TOKEN`。目录每天按 `Asia/Shanghai` 03:00 增量同步，GitHub 失败时继续提供最后一次成功快照。
