...
**Architecture:** 新建后端 `skills/` 包(schemas/parser/store/engine,与三引擎平级),复用 `AgentLoop`(ReAct)执行技能正文;意图路由扩展第四类 "skill"(embedding 吃 `when_to_use` 向量 + LLM 分类吃候选列表 + forced 显式分支);前端 Composer 加「技能」chip + Popover + 导入弹窗。规范源 `docs/skills/skills-design-v1.md`,实现定稿 `docs/skills/skills-design-v2.md`(三分叉 A1/B1/C1 + 15 条 drift 对齐 + 前置断言落地)。
...
### Task 5.1: demo 技能 + `../api-contract/api-contract-v2.md` 追加

**Files:**
- Create: `docs/demo-skills/capacity-report.md`(可导入的 demo 技能)
- Modify: `docs/api-contract/api-contract-v2.md`(追加端点总览三行、§1 `skill_id` 字段、新小节「技能模块」)
...
- [ ] **Step 2: Append to `docs/api-contract/api-contract-v2.md`** (照 v1 §5):
...
3. 新小节「技能模块 (Skills)」:`SkillMeta` 形状(字段表)、multipart 约定(`file` 字段,.md/.zip)、错误语义(413/415/422/409)、frontmatter 规范链接 `docs/skills/skills-design-v1.md` §2。

- [ ] **Step 3: Commit**

```bash
git add docs/demo-skills/capacity-report.md docs/api-contract/api-contract-v2.md
...
Plan complete and saved to `docs/skills/skills-implementation-plan.md`. Two execution options:
